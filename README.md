...
### Трудности
- Чтение stout/err в режиме реального времени (как потом оказалось, мешала буферизация питоновского процесса, которую было не отключить при запуске процесса через subprocess), на помощь пришел флаг '-u' при запуске интерпретатора.
